...
#define GME_VERSION 0x000606 /* 1 byte major, 1 byte minor, 1 byte patch-level */
...
{
	int enabled;      /* control bit7: oscillator enabled */
	int dda_mode;     /* control bit6: direct D/A mode (no tone, raw DAC writes) */
	int noise_on;     /* noise bit7 (channels 4/5 only): noise mode enabled */
	int channel_vol;  /* control bits4-0: channel volume, 0-31 */
	int balance;      /* raw balance register: bits7-4 = left weight, bits3-0 = right weight */
	int period;       /* 12-bit waveform period register (0 = silent/keycode invalid) */
	int noise_freq;   /* noise bits4-0 (channels 4/5 only): noise frequency, 0-31 */
	int gain_l;       /* synthesized final linear gain, left */
	int gain_r;       /* synthesized final linear gain, right */
