...
void Hes_Apu::get_osc_state( int index, gme_hes_channel_state_t* out ) const
{
	...
	out->enabled     = (unsigned char) ((osc.control & 0x80) != 0);
	out->dda_mode    = (unsigned char) ((osc.control & 0x40) != 0);
	out->noise_on    = (unsigned char) ((osc.noise   & 0x80) != 0);
	out->channel_vol = (unsigned char) (osc.control & 0x1F);
	out->balance     = osc.balance;
	out->period      = (unsigned short) osc.period;
	out->noise_freq  = (unsigned char) (osc.noise & 0x1F);
